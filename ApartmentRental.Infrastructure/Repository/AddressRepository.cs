...
namespace ApartmentRental.Infrastructure.Repository
{
    public class AddressRepository : IAddressRepository
    {
        ...
        {
            var address = await (_mainContext.Address ?? throw new InvalidOperationException()).SingleOrDefaultAsync(x => x.Id == id);
            if (address == null)
            {
                throw new EntityNotFoundException();
            }
        ...
        {
            var address = await (_mainContext.Address ?? throw new InvalidOperationException()).FirstOrDefaultAsync(x =>
